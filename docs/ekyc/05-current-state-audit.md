...
|---|---|---|---|
| Document OCR + MRZ | ✅ built | `ekyc-ml-service/engine/ocr.py`, `engine/mrz.py` (ICAO 9303 TD1+TD3, per-check-digit), `api/document.py:9-61` | Tesseract `eng` only — no `amh` traineddata in `Dockerfile:11`; accuracy on real (non-synthetic) documents unmeasured |
