...
- ⏳ **B.3** origination — enforce `checker != maker` on loan approve/disburse + config (incl. per-product flag). [parallel agent]
- ⏳ **B.4** UI — approval-queue page + config screen; deposit/withdraw/transfer/close handle the 202 "pending approval" response. [parallel agent]
