...
**Remaining prevention follow-ups (recommended, not yet done):** (2) make `connection.go` keep retrying rather than cap at 10 attempts/20s; (3) gate pod **readiness** on broker connectivity (probe / initContainer) so a publish-incapable pod isn't marked Ready; (4) **transactional outbox** for durable event delivery (kills the dual-write at the root); (5) reconciliation job: `DISBURSED` applications with no loan in loan-management → re-emit/flag, plus a publish-failure metric/alert; (6) regression guard that fails fast if disburse doesn't yield an ACTIVE loan.
