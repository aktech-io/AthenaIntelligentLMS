...
-- where a committed account state change is followed by a lost broker publish —
-- see F27 / docs/EDA_HARDENING.md). Money-path events (account.credit.received,
-- account.debit.processed, transfer.completed) are written into this table in
-- the SAME tx as the balance change and delivered at-least-once by the relay.

CREATE TABLE IF NOT EXISTS event_outbox (
    id              BIGSERIAL   PRIMARY KEY,
    event_id        UUID        NOT NULL,            -- DomainEvent.ID; consumer idempotency key
    event_type      TEXT        NOT NULL,            -- routing key, e.g. 'account.credit.received'
    aggregate_id    TEXT,                            -- optional: account/transfer id (debug only)
    tenant_id       TEXT,
