-- Tamper-evident audit trail for overdraft_audit_log: hash-chain every entry and
-- make the table append-only, so an auditor can cryptographically prove the log
-- was not altered or back-dated. Each row's entry_hash = SHA256(prev_hash ||
-- canonical content); deleting or editing any past row breaks the chain.
--
-- Adapted from the account-service template (migrations/account/000013) to this
-- table's actual columns. See docs/AUDIT_TAMPER_EVIDENCE.md.

CREATE EXTENSION IF NOT EXISTS pgcrypto;

ALTER TABLE overdraft_audit_log ADD COLUMN IF NOT EXISTS seq        BIGSERIAL;
ALTER TABLE overdraft_audit_log ADD COLUMN IF NOT EXISTS prev_hash  TEXT;
ALTER TABLE overdraft_audit_log ADD COLUMN IF NOT EXISTS entry_hash TEXT;
...
CREATE OR REPLACE FUNCTION audit_canonical(
    ...
    p_actor TEXT, p_before TEXT, p_after TEXT, p_metadata TEXT,
    p_created TIMESTAMPTZ
) RETURNS TEXT AS $$
    SELECT concat_ws('|',
        p_tenant, p_action, p_etype, p_eid,
        coalesce(p_actor,''),
        coalesce(p_before,''), coalesce(p_after,''), coalesce(p_metadata,''),
...
BEGIN
    FOR rec IN
        SELECT * FROM overdraft_audit_log WHERE entry_hash IS NULL ORDER BY tenant_id, seq
    LOOP
        ...
        v_hash := encode(digest(
            v_prev || '|' || audit_canonical(rec.tenant_id, rec.action, rec.entity_type,
                rec.entity_id::text, rec.actor,
                rec.before_snapshot::text, rec.after_snapshot::text, rec.metadata::text,
                rec.created_at),
            'sha256'), 'hex');
        UPDATE overdraft_audit_log SET prev_hash = v_prev, entry_hash = v_hash WHERE id = rec.id;
        v_prev := v_hash;
    END LOOP;
END $$;
...
DECLARE
    v_prev TEXT;
BEGIN
    PERFORM pg_advisory_xact_lock(hashtext('overdraft_audit_log:' || NEW.tenant_id));
    SELECT entry_hash INTO v_prev
        FROM overdraft_audit_log
    ...
    NEW.entry_hash := encode(digest(
        v_prev || '|' || audit_canonical(NEW.tenant_id, NEW.action, NEW.entity_type,
            NEW.entity_id::text, NEW.actor,
            NEW.before_snapshot::text, NEW.after_snapshot::text, NEW.metadata::text,
            NEW.created_at),
        'sha256'), 'hex');
    RETURN NEW;
END;
...
CREATE TRIGGER trg_audit_hash BEFORE INSERT ON overdraft_audit_log
...
BEGIN
    RAISE EXCEPTION 'overdraft_audit_log is append-only — % is not permitted', TG_OP;
END;
$$ LANGUAGE plpgsql;

DROP TRIGGER IF EXISTS trg_audit_no_update ON overdraft_audit_log;
CREATE TRIGGER trg_audit_no_update BEFORE UPDATE ON overdraft_audit_log
    FOR EACH ROW EXECUTE FUNCTION audit_block_modify();
DROP TRIGGER IF EXISTS trg_audit_no_delete ON overdraft_audit_log;
CREATE TRIGGER trg_audit_no_delete BEFORE DELETE ON overdraft_audit_log
    FOR EACH ROW EXECUTE FUNCTION audit_block_modify();
...
DECLARE
    ...
    v_total BIGINT := 0;
BEGIN
    FOR rec IN SELECT * FROM overdraft_audit_log WHERE tenant_id = p_tenant ORDER BY seq LOOP
        v_total := v_total + 1;
