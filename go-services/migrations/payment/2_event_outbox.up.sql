...
-- where a committed payment state change is followed by a lost broker publish —
-- see F27 / docs/EDA_HARDENING.md). Money-path events (payment.completed) are
-- written into this table in the SAME tx as the state change and delivered
-- at-least-once by the outbox relay.

CREATE TABLE IF NOT EXISTS event_outbox (
    id              BIGSERIAL   PRIMARY KEY,
    event_id        UUID        NOT NULL,            -- DomainEvent.ID; consumer idempotency key
    event_type      TEXT        NOT NULL,            -- routing key, e.g. 'payment.completed'
    aggregate_id    TEXT,                            -- optional: payment id (debug only)
    tenant_id       TEXT,
