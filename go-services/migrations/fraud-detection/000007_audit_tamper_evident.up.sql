-- Tamper-evident audit trail for fraud_audit_log: hash-chain every entry and
...
ALTER TABLE fraud_audit_log ADD COLUMN IF NOT EXISTS seq        BIGSERIAL;
ALTER TABLE fraud_audit_log ADD COLUMN IF NOT EXISTS prev_hash  TEXT;
ALTER TABLE fraud_audit_log ADD COLUMN IF NOT EXISTS entry_hash TEXT;
...
CREATE OR REPLACE FUNCTION audit_canonical(
    ...
    p_performed_by TEXT, p_description TEXT, p_changes TEXT,
    p_created TIMESTAMPTZ
) RETURNS TEXT AS $$
    SELECT concat_ws('|',
        p_tenant, p_action, p_etype, p_eid,
        coalesce(p_performed_by,''), coalesce(p_description,''), coalesce(p_changes,''),
...
BEGIN
    FOR rec IN
        SELECT * FROM fraud_audit_log WHERE entry_hash IS NULL ORDER BY tenant_id, seq
    LOOP
        ...
        v_hash := encode(digest(
            v_prev || '|' || audit_canonical(rec.tenant_id, rec.action, rec.entity_type,
                rec.entity_id::text, rec.performed_by, rec.description, rec.changes::text,
                rec.created_at),
            'sha256'), 'hex');
        UPDATE fraud_audit_log SET prev_hash = v_prev, entry_hash = v_hash WHERE id = rec.id;
        v_prev := v_hash;
    END LOOP;
END $$;
...
DECLARE
    v_prev TEXT;
BEGIN
    PERFORM pg_advisory_xact_lock(hashtext('fraud_audit_log:' || NEW.tenant_id));
    ...
    NEW.entry_hash := encode(digest(
        v_prev || '|' || audit_canonical(NEW.tenant_id, NEW.action, NEW.entity_type,
            NEW.entity_id::text, NEW.performed_by, NEW.description, NEW.changes::text,
            NEW.created_at),
        'sha256'), 'hex');
    RETURN NEW;
END;
...
DROP TRIGGER IF EXISTS trg_audit_hash ON fraud_audit_log;
CREATE TRIGGER trg_audit_hash BEFORE INSERT ON fraud_audit_log
...
BEGIN
    RAISE EXCEPTION 'fraud_audit_log is append-only — % is not permitted', TG_OP;
END;
$$ LANGUAGE plpgsql;

DROP TRIGGER IF EXISTS trg_audit_no_update ON fraud_audit_log;
CREATE TRIGGER trg_audit_no_update BEFORE UPDATE ON fraud_audit_log
    FOR EACH ROW EXECUTE FUNCTION audit_block_modify();
DROP TRIGGER IF EXISTS trg_audit_no_delete ON fraud_audit_log;
CREATE TRIGGER trg_audit_no_delete BEFORE DELETE ON fraud_audit_log
    FOR EACH ROW EXECUTE FUNCTION audit_block_modify();
...
DECLARE
    ...
    v_total BIGINT := 0;
BEGIN
    FOR rec IN SELECT * FROM fraud_audit_log WHERE tenant_id = p_tenant ORDER BY seq LOOP
        v_total := v_total + 1;
