...
-- that silently dropped loan.disbursed — see F27 / docs/EDA_HARDENING.md).
--
-- Shared by loan-origination and loan-management (both use athena_loans).

CREATE TABLE IF NOT EXISTS event_outbox (
    id              BIGSERIAL   PRIMARY KEY,
    event_id        UUID        NOT NULL,            -- DomainEvent.ID; consumer idempotency key
    event_type      TEXT        NOT NULL,            -- routing key, e.g. 'loan.disbursed'
    aggregate_id    TEXT,                            -- optional: loan/application id (debug only)
    tenant_id       TEXT,
